...
static void InitializeTCPStreamListener(HAPPlatformTCPStreamListener* tcpStreamListener) {
    ...
    tcpStreamListener->fileDescriptor = INVALID_SOCKET;
...
static void InitializeTCPStream(HAPPlatformTCPStream* tcpStream) {
    ...
    tcpStream->fileDescriptor = INVALID_SOCKET;
 ...
 *
 * @param      fileDescriptor       Socket descriptor.
 *
 * @return kHAPError_None           If successful.
 * @return kHAPError_Unknown        If the nonblocking flag could not be set.
 */
HAP_RESULT_USE_CHECK
static HAPError SetNonblocking(SOCKET fileDescriptor) {
    u_long mode = 1;
    int result = ioctlsocket(fileDescriptor, FIONBIO, &mode);
    if (result != 0) {
        HAPPlatformLogWindowsError(
                kHAPLogType_Error,
                "System call 'ioctlsocket' to set socket to 'non-blocking' failed.",
                WSAGetLastError(),
                __func__,
                HAP_FILE,
                __LINE__);
 ...
 */
HAP_RESULT_USE_CHECK
static HAPError SetNodelay(SOCKET fileDescriptor) {
    int v = 1;
    HAPLogBufferDebug(
            &logObject, &v, sizeof v, "setsockopt(%llu, %d, %d, <buffer>);",
            (unsigned long long)fileDescriptor, IPPROTO_TCP, TCP_NODELAY);
    int e = setsockopt(fileDescriptor, IPPROTO_TCP, TCP_NODELAY, (const char*)&v, sizeof v);
    if (e != 0) {
        int wsaError = WSAGetLastError();
        HAPAssert(e == SOCKET_ERROR);
        HAPPlatformLogWindowsError(
                kHAPLogType_Error,
...
void HAPPlatformTCPStreamManagerCreate(
    ...
    // Initialize Winsock (WSAStartup is called in HAPPlatformRunLoopCreate)
}
...
void HAPPlatformTCPStreamManagerOpenListener(
    ...
    HAPPrecondition(tcpStreamManager->tcpStreamListener.fileDescriptor == INVALID_SOCKET);
    ...
    if (tcpStreamManager->tcpStreamListenerConfiguration.interfaceName[0]) {
        // Windows does not have if_nametoindex, so we use interface index 0 (all interfaces)
        HAPLog(&logObject, "Ignoring local network interface name on Windows - binding to all interfaces.");
        interfaceIndex = 0;
    } else {
        interfaceIndex = 0;
    }
    ...
    SOCKET fileDescriptor = socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP);
    ...
    int v = 1;
    HAPLogBufferDebug(&logObject, &v, sizeof v, "setsockopt(%llu, SOL_SOCKET, SO_REUSEADDR, <buffer>);",
                      (unsigned long long)fileDescriptor);
    e = setsockopt(fileDescriptor, SOL_SOCKET, SO_REUSEADDR, (const char*)&v, sizeof v);
    if (e != 0) {
    ...
    HAPLogBufferDebug(&logObject, (struct sockaddr*) &sin6, sizeof sin6, "bind(%llu, <buffer>);",
                      (unsigned long long)fileDescriptor);
    ...
    HAPLogDebug(&logObject, "listen(%llu, 64);", (unsigned long long)fileDescriptor);
    ...
    tcpStreamManager->tcpStreamListener.fileDescriptor = (int)fileDescriptor;
...
void HAPPlatformTCPStreamManagerCloseListener(HAPPlatformTCPStreamManagerRef tcpStreamManager) {
    ...
    SOCKET sock = (SOCKET)tcpStreamManager->tcpStreamListener.fileDescriptor;
    HAPLogDebug(&logObject, "shutdown(%llu, SD_BOTH);", (unsigned long long)sock);
    e = shutdown(sock, SD_BOTH);
    if (e != 0) {
        int wsaError = WSAGetLastError();
        HAPAssert(e == SOCKET_ERROR);
        HAPPlatformLogWindowsError(
    ...
    HAPLogDebug(&logObject, "closesocket(%llu);", (unsigned long long)sock);
    e = closesocket(sock);
    if (e != 0) {
        int wsaError = WSAGetLastError();
        HAPAssert(e == SOCKET_ERROR);
        HAPPlatformLogWindowsError(
                kHAPLogType_Debug,
                "System call 'closesocket' on TCP stream listener socket failed.",
                wsaError,
                __func__,
                HAP_FILE,
                __LINE__);
...
HAPError HAPPlatformTCPStreamManagerAcceptTCPStream(
    ...
    size_t i = 0;
    while ((i < tcpStreamManager->maxTCPStreams) &&
           (tcpStreamManager->tcpStreams[i].fileDescriptor != INVALID_SOCKET)) {
        i++;
    }
    ...
    HAPAssert(tcpStream->fileDescriptor == INVALID_SOCKET);
    HAPAssert(!tcpStream->fileHandle);

    SOCKET listenerSocket = (SOCKET)tcpStreamManager->tcpStreamListener.fileDescriptor;
    HAPLogDebug(&logObject, "accept(%llu, NULL, NULL);", (unsigned long long)listenerSocket);
    SOCKET fileDescriptor = accept(listenerSocket, NULL, NULL);
    if (fileDescriptor == INVALID_SOCKET) {
        int wsaError = WSAGetLastError();
        if (wsaError != WSAEWOULDBLOCK && wsaError != WSAEINTR && wsaError != WSAECONNABORTED) {
            HAPPlatformLogWindowsError(
                    kHAPLogType_Error,
    ...
    tcpStream->fileDescriptor = (int)fileDescriptor;
...
void HAPPlatformTCPStreamCloseOutput(
    ...
    HAPPrecondition(tcpStream->fileDescriptor != INVALID_SOCKET);
    HAPPrecondition(tcpStream->fileHandle);

    SOCKET sock = (SOCKET)tcpStream->fileDescriptor;
    HAPLogDebug(&logObject, "shutdown(%llu, SD_SEND);", (unsigned long long)sock);
    int e = shutdown(sock, SD_SEND);
    if (e != 0) {
        int wsaError = WSAGetLastError();
        HAPAssert(e == SOCKET_ERROR);
        HAPPlatformLogWindowsError(
...
void HAPPlatformTCPStreamClose(HAPPlatformTCPStreamManagerRef tcpStreamManager, HAPPlatformTCPStreamRef tcpStream_) {
    ...
    if (e != 0) {
        ...
        HAPPlatformLogWindowsError(
                kHAPLogType_Debug,
                "System call 'closesocket' on TCP stream socket failed.",
                wsaError,
                __func__,
                HAP_FILE,
                __LINE__);
    ...
    if (tcpStreamManager->tcpStreamListener.fileDescriptor != INVALID_SOCKET) {
...
HAPError HAPPlatformTCPStreamRead(
    ...
    SOCKET sock = (SOCKET)tcpStream->fileDescriptor;
    int n = recv(sock, (char*)bytes, (int)maxBytes, 0);
...
HAPError HAPPlatformTCPStreamWrite(
    ...
    SOCKET sock = (SOCKET)tcpStream->fileDescriptor;
    int n = send(sock, (const char*)bytes, (int)maxBytes, 0);
...
static void HandleTCPStreamListenerFileHandleCallback(
    ...
    HAPAssert(listener->fileDescriptor != INVALID_SOCKET);
